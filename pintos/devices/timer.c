...
void wake_up_sleeping_thread (int64_t now) {                

	while (!list_empty(&sleep_list)) {                  
		ASSERT (intr_get_level () == INTR_OFF);
		struct thread *t = list_entry(t, struct thread, elem);
