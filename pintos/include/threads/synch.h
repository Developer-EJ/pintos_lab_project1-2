...
bool sema_try_down (struct semaphore *);                   // 값이 성공적으로 감소 = true, 값이 이미 0이어서 감소시킬 수 없다면 false 반환
...
bool lock_held_by_current_thread (const struct lock *);   // 현재 실행 중인 스레드가 해당 락을 소유하고 있다면 True를 반환, 그렇지 않다면 false 반환
...
void cond_wait (struct condition *, struct lock *);      // lock을 원자적으로 해제한 뒤, 다른 코드가 cond 신호를 보내기를 기다림
void cond_signal (struct condition *, struct lock *);    // cond 신호를 기다리는 스레드들 중 하나를 깨움
void cond_broadcast (struct condition *, struct lock *); // cond 신호를 기다리는 모든 스레드들 깨움
